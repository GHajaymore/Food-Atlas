...
export const EN: Copy = {
  ...
  privacyCookiesBody: "Two cookies, both signed and neither readable by scripts on the page. One keeps a confirmation counting once per person. The other exists only if you sign in with Google — and even then the app asks Google for one thing: that you are the same person as last time. No email address, no name, and no picture. Signing in writes no record of you at all: the only accounts this database stores are the handful who run the site, kept as that same hash with a role beside it. Your chosen language is remembered inside your own browser and never sent anywhere.",
